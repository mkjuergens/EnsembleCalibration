...
def get_soft_binning_ece_tensor(predictions: torch.Tensor, labels: torch.Tensor, n_bins: int, soft_binning_temp: float,
    ...
    soft_anchors = torch.Tensor(np.arange(1.0/(2.0 * n_bins), 1.0, 1.0 / n_bins), dtype=torch.Tensor.float32)
...
def calculate_pbar_torch(weights_l: torch.Tensor, p_preds: torch.Tensor, reshape: bool = False):
    ...
    if reshape:
        assert len(weights_l) % n_rows == 0, " weight vector needs to be a multiple of the "  
        "number of rows"
        weights_l = weights_l.reshape(n_rows, -1)

    assert weights_l.shape[0] == p_preds.shape[0], " numer of samples need to be the same for P and weights_l"
    assert weights_l.shape[1] == p_preds.shape[1], " numer of ensemble members need to be the same for P and weights_l"
