...
#     for _ in tqdm(range(cfg.n_resamples), desc=desc):
#         x, P, _, y, _ = call_with_filtered_kwargs(
#             sampler,
#             vars(cfg),
#             h0=h0,
#             x_dep=x_dep,
...
#                 if k in signature(bootstrap_test).parameters
#             }
#             _, p_vals, _ = bootstrap_test(**filt)
#             delta = np.asarray(p_vals, float)
#             results[name] += delta if h0 else 1.0 - delta

#     return [(results[n] / cfg.n_resamples).tolist() for n in obj_map]
...
def simulate(
    ...
    for _ in tqdm(range(cfg.n_resamples), desc=f"{'H0' if h0 else 'H1'}-{ 'x' if x_dep else 'const'}"):
...
def main(args: Any) -> None:
    ...
    save_dir = os.path.join(args.results_dir, cfg.experiment, timestamp)
    os.makedirs(save_dir, exist_ok=True)

    # ---- H0 simulations ----
    alpha_h0 = [0.05 * i for i in range(1, 13)]
    ...
    csv_names = ["h0_const.csv", "h0_xdep.csv"] + [
        f"h1_deg{d}.csv" for d in cfg.deg_h1_list
    ]
    df_full = pd.concat(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--bootstrap_test", choices=["v0", "v2"], default="v0"
    )
    parser.add_argument(
        "--objectives",
        nargs="*",
